...
func TestRedisStore_HeartbeatRecordsRow(t *testing.T) {
	s := openTestStore(t)
	ctx := t.Context()
	now := time.Now().UTC().Truncate(time.Microsecond)
...
func TestRedisStore_RecoverStale(t *testing.T) {
	s := openTestStore(t)
	ctx := t.Context()
	now := time.Now().UTC().Truncate(time.Microsecond)
	_ = s.Save(ctx, gs.Job{ID: "j", Queue: "default", Name: "n", RunAt: now, State: gs.StatePending, MaxAttempts: 3})
...
func TestRedisStore_QueueSize(t *testing.T) {
	s := openTestStore(t)
	ctx := t.Context()
	now := time.Now()
...
func TestRedisStore_RecurringCRUD(t *testing.T) {
	s := openTestStore(t)
	ctx := t.Context()
...
func TestRedisStore_UpdateRecurringNextRun(t *testing.T) {
	s := openTestStore(t)
	ctx := t.Context()
	now := time.Now().UTC().Truncate(time.Microsecond)
